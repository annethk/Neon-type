...
ts e a lógica "mock" que existiam antes aqui:
// Jogador, Fase, BancoPalavras/Palavra, TimerFase, Ranking e a loja
// (economia) agora vêm prontos desses headers, em vez de serem
// redeclarados e simulados localmente no main.c.
#include "jogador.h"
            ...
            switch (telaAtual) {
                ...
                case GAMEPLAY:
                    ...
                    for (int i = 0; i < tamP; i++) {
                        char letra[2] = { palavraAtual[i], '\0' };
                        if (i < tamI) {
                            //desenha a letra em branco
                            if (input[i] == palavraAtual[i]) DrawText(letra, centroX + (i * 8), centroY, 14, corLetraAcertada);
                            //se a letra for errada, desenha vermelho
                            else DrawText(letra, centroX + (i * 8), centroY, 14, RED);
                            } else {
                                //se a letra for certa, desenha verde
                            DrawText(letra, centroX + (i * 8), centroY, 14, DARKGREEN);
                            }
                    }
                    //Rodape da tela de gameplay
                    DrawText(TextFormat(" %s_", input), 73, 196, 12, WHITE); 
                    DrawText(TextFormat("x%d", jogador.combo), 185, 225, 12, (jogador.combo >= 20)? corNeonBase : GREEN);
                    DrawText(TextFormat("%d", faseAtual + 1), 292, 226, 12, corNeonBase);
